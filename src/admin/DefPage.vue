    ...
    methods: {
      ...
      btnedit: function (row) {
        ...
        if (!u.isnullorwhitespace(row)) {
          u.getmodel(this, 'Def', row.pk, 'mm', '编辑')
        }
      },
      btnsave: function (row) {
        let that = this
        u.getdata(that, 'Def', u.isnullorwhitespace(that.mm.ts) ? 'Add' : 'Update', that.mm).then(function (r) {
          if (r.ok) {
      ...
      btndelete: function () {
        u.messagedelete(this).then(() => {
          u.pagebatchoperate(this, 'Def', 'Delete')
        }).catch((e) => {
          console.log(e)
        })
      },
